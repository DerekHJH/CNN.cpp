	...
	for (iter = 0; iter < num_epochs_CNN; iter++) {
		...
		for (int i = 0; i < num_patterns_train_CNN; i++) {
			...
			if (i % 1000 == 0) {
				gettimeofday(&tsEnd, NULL);
				t1Duration = 1000000L * (tsEnd.tv_sec - tsBegin.tv_sec) + (tsEnd.tv_usec - tsBegin.tv_usec);
				// We do not print t1Duration / 1000, because we use the time of a single step to
				// represent 1000 steps. And the total time of 1000 steps should be multiplied.
		...
		// If we make it to accuracy_rate_CNN, then we can stop early.
